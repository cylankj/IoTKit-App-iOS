...
- (long long)fileSizeInCachePlistWithDownloadModel:(TYDownloadModel *)downloadModel
{
    ...
    return [downloadsFileSizePlist[[[NSURL URLWithString:downloadModel.downloadURL] lastPathComponent]] longLongValue];
}
...
- (NSString *)fileSizePathWithDownloadModel:(TYDownloadModel *)downloadModel
{
    ...
    if (![downloadModel.filePath isEqualToString:@""] && downloadModel.filePath != nil)
    {
        plistPath = [[downloadModel.filePath stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"downloadsFileSize.plist"];
        
    }
    else
    {
        plistPath = [self.downloadDirectory stringByAppendingString:@"downloadsFileSize.plist"];
    }
    return plistPath;
}
...
- (BOOL)isDownloadCompletedWithDownloadModel:(TYDownloadModel *)downloadModel
{
    ...
    NSLog(@"fileTotalSize [%lld]   downloaded size [%lld]", fileSize, [self fileSizeWithDownloadModel:downloadModel]);
...
totalBytesExpectedToWrite:(int64_t)totalBytesExpectedToWrite
{
    ...
    NSMutableDictionary *fileSize = [self fileSizePlistWithDownloadModel:downloadModel];
    if ([[fileSize allKeys] containsObject:[[NSURL URLWithString:downloadModel.downloadURL] lastPathComponent]])
    {
        if ([fileSize[[[NSURL URLWithString:downloadModel.downloadURL] lastPathComponent]] longLongValue] != totalBytesExpectedToWrite)
        {
            fileSize[[[NSURL URLWithString:downloadModel.downloadURL] lastPathComponent]] = @(totalBytesExpectedToWrite);
            [fileSize writeToFile:[self fileSizePathWithDownloadModel:downloadModel] atomically:YES];
        }
    }
    else
    {
