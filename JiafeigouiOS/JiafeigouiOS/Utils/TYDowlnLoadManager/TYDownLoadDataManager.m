    ...
    if ([self.fileManager fileExistsAtPath:downloadModel.filePath]) {
        ...
        if ([self.fileManager fileExistsAtPath:[self fileSizePathWithDownloadModel:downloadModel]]) {
            @synchronized (self) {
                ...
                [dict removeObjectForKey:[[NSURL URLWithString:downloadModel.downloadURL] lastPathComponent]];
...
- (void)URLSession:(NSURLSession *)session dataTask:(NSURLSessionDataTask *)dataTask didReceiveResponse:(NSHTTPURLResponse *)response completionHandler:(void (^)(NSURLSessionResponseDisposition))completionHandler
{
    ...
    @synchronized (self) {
        ...
        dic[[[NSURL URLWithString:downloadModel.downloadURL] lastPathComponent]] = @(totalBytesExpectedToWrite);
...
- (void)URLSession:(NSURLSession *)session dataTask:(NSURLSessionDataTask *)dataTask didReceiveData:(NSData *)data
{
    ...
    // 写入数据
    [downloadModel.stream write:static_cast<const uint8_t *>(data.bytes) maxLength:data.length];
